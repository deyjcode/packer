...
Write-Output "Validating Azure Datacenter GUI Template"
Start-Process 'packer' -ArgumentList "validate -var `"client_id=$($azurepackerdata.client_id)`" -var `"client_secret=$($azurepackerdata.client_secret)`" -var `"tenant_id=$($azurepackerdata.tenant_id)`" -var `"subscription_id=$($azurepackerdata.subscription_id)`" -var `"managed_image_resource_group_name=$($azurepackerdata.managed_image_resource_group_name)`" -var `"managed_image_storage_account_type=$($azurepackerdata.managed_image_storage_account_type)`" -var `"os_type=$($azurepackerdata.os_type)`" -var `"image_publisher=$($azurepackerdata.image_publisher)`" -var `"image_offer=$($azurepackerdata.image_offer)`" -var `"winrm_username=$($azurepackerdata.winrm_username)`" -var `"location=$($azurepackerdata.location)`" -var `"vm_size=$($azurepackerdata.vm_size)`" -var `"image_sku=2016-Datacenter`" .\azure_2016std.json" -Wait -NoNewWindow

# Build 2016 Datacenter Core
Write-Output "Validating Azure Datacenter Core Template"
